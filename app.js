...
async function getConcerts(query = "") {
  try {
    ...
    renderCarousel(filtered); // actualizar carrusel
  } catch (err) {
  ...
  events.slice(0, 5).forEach((ev, index) => {
    ...
    carousel.innerHTML += `
      <div class="carousel-item ${active}">
        <img src="${ev.images[0].url}" class="d-block w-100" alt="${ev.name}" style="height: 400px; object-fit: cover; cursor:pointer;" onclick='openModal(${JSON.stringify(ev)})'>
        <div class="carousel-caption d-none d-md-block bg-dark bg-opacity-50 rounded">
          <h5>${ev.name}</h5>
          <p>${ev._embedded.venues[0].city.name}, ${ev.dates.start.localDate}</p>
        </div>
      </div>
    `;
  });
